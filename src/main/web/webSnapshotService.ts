...
import { extractPdfText } from '../infra/pdfTextService'
...
export async function getWebSnapshotDocumentContext(
  ...
  let body = ''
  try {
    body = await extractPdfText(pdfPath)
  } catch {
    body = ''
  }
  if (!body.trim()) {
    body =
      '（网页 PDF 快照未能提取正文，可能是纯图片页面。请使用便签标注，或在有文本层时选中文字 Ask AI。）'
  }

  const content = `${sourceLine}${body}`
